...
#list of IP addresses for the hosts to be checked
#The short list of hosts is used for testing
#POSs = ['10.2.12.164', '10.2.12.158', '10.2.12.160']
...
for is_sales_POS in is_sales_pos_list:
    
    file_name = r"{}_cfg.txt".format(is_sales_POS)
    ...
    try:
        ...
        with open (file_path1, "r", encoding='unicode-escape') as file2:
            ...
            pattern = r"(DirectNet\=)(\d+\.\d+\.\d+\.\d+)(\:)(3202)"
            retd_pattern = re.search(pattern, Directnet_text)    
            if retd_pattern[0] != None:
                correct_list.append(is_sales_POS)
            else:
                continue

                
    except:
        print("Error during regex parsing of text for host {}.  Continuing...".format(is_sales_POS))
        continue

#This workaround adds the salespoint to the incorrect list if it does not have the proper port.  
for element in is_sales_pos_list:
    if element not in correct_list:
...
if correct_str2:
    with open (filepath, 'a', 0o700) as file3:
        file3.write(r"Sales host(s) {} were online when the program ran.  They are using the correct port for DirectNet (3202).  If nothing else is below this message, no hosts were found to be using the incorrect port at this time.  ".format(correct_str2))
...
if incorrect_str2:
        with open (filepath, 'a', 0o700) as file3:
            file3.write(r"WARNING: Sales host(s) {} may be using the incorrect port for DirectNet and this may cause accounting errors if not resolved.  Please notify Purgatory I.T. and they will correct the errors.  ".format(incorrect_str2))
...
youse = [r'gashley@purgatory.ski', r'itsupport@purgatory.ski', r'cjans@purgatory.ski']

#youse = ['gashley@purgatory.ski']
...
#The try/except block below allows the script to still exit in the case that there is no file at that path to delete.
#The below try/except block is to handle the case where there are no files/directories to delete, which caused the script to finish with errors in testing.  
try:
    shutil.rmtree(dir_path)
    os.remove(file_name1)
except:
    print(r"some error occurred right at the end of the script.")
    exit()
print('all done now.')
exit()
